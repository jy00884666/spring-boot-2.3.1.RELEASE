...
class OriginTrackedPropertiesLoader {
	 ...
	 * {@link OriginTrackedValue}.
	 *
	 * 加戴 properties 文件的数据并返 map 类型
	 * 其中 expandLists 用于指定参数为"name[]=a, b,c"的列表是否进行扩展，默 true
	 *
	...
	private String loadKey(StringBuilder buffer, CharacterReader reader) throws IOException {
		...
		while (!reader.isEndOfLine()) {
			// 是否是分隔符,reader.isPropertyDelimiter用来判断当前字符是否是key/value分隔符，如果是则说明已经读取到完整的key，继续读取下一个字符，直到读取到完整的key
